document.addEventListener("DOMContentLoaded", async function () {
  ...
  document
    ...
    .addEventListener("submit", handleSupply);
  document
    .getElementById("supplyProduct")
    .addEventListener("change", updateWarehouseInfo);
});
...
async function handleSupply(e) {
  ...
  try {
    ...
    if ((warehouseData?.quantity || 0) < quantity) {
      alert(
        `الكمية المتاحة (${warehouseData?.quantity || 0}) أقل من الكمية المطلوبة (${quantity})`,
      );
      return;
    }
...
async function loadWarehouseStock() {
  try {
    ...
    if (data.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-center text-muted">لا توجد منتجات في المخزن</td></tr>';
      return;
    }
