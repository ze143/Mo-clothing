...
async function loadTransfers() {
  try {
    ...
    var query = supabaseClient.from("branch_transfers").select("*");
...
function showSupplyModal() {
  ...
  // ✅ تحديث الكمية المتاحة عند تغيير المنتج
  document
...
// ============================================================
// ✅ توريد من المخزن للفرع (مع تسجيل تلقائي في branch_transfers)
// ============================================================
...
async function executeSupply() {
  ...
  try {
    ...
    // ✅ 2. تسجيل التوريد في branch_transfers (موجود بالفعل)
    var { data: userData } = await supabaseClient.auth.getUser();
    var userId = userData?.user?.id || null;
    ...
    // ✅ 3. تحديث الفلتر لعرض التوريد الجديد
    setTimeout(function () {
      supplyModal.hide();
      loadTransfers();
    }, 1500);
