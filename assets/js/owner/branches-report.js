...
async function loadBranchesReport() {
    try {
        // جلب الفروع
        const branchesResult = await supabaseClient.from("branches").select("*").order("name");
        ...
        if (!branches || branches.length === 0) {
            container.innerHTML = '<div class="text-center text-muted py-4">لا توجد فروع</div>';
            return;
        }
        ...
        for (var i = 0; i < branches.length; i++) {
            ...
            var branchStock = stock.filter(function(s) { return s.branch_id === branch.id; });
            var branchTotal = branchStock.reduce(function(sum, s) { return sum + (s.quantity || 0); }, 0);
            ...
            if (branchStock.length === 0) {
                itemsHtml = '<tr><td colspan="3" class="text-center text-muted">لا توجد منتجات</td></tr>';
            } else {
                for (var j = 0; j < branchStock.length; j++) {
                    var item = branchStock[j];
                    var productName = (item.products && item.products.name) ? item.products.name : "غير معروف";
                    itemsHtml += '<tr><td>' + (j + 1) + '</td><td>' + productName + '</td><td><span class="badge bg-primary">' + item.quantity + '</span></td></tr>';
                }
            }
