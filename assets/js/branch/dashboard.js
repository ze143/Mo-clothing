// ============================================================
// 📦 جلب الكمية المتاحة في الفرع (نسخة مُصلحة)
// ============================================================

async function getAvailableStock(productId) {
  try {
    // ✅ تأكد إن currentBranchId موجود
    if (!currentBranchId) {
      console.error("❌ currentBranchId غير موجود");
      return 0;
    }

    // ✅ جلب الكمية من branch_stock
    ...
    if (error) {
      console.error("❌ خطأ في جلب الكمية:", error.message);
      return 0;
    }

    // ✅ لو مفيش سجل، يبقى 0
    var quantity = (data && data.quantity) || 0;

    // ✅ عرض في Console للتأكد
    console.log("📦 getAvailableStock(" + productId + ") = " + quantity);

    return quantity;
  } catch (error) {
    console.error("❌ خطأ في getAvailableStock:", error);
    return 0;
  }
}
// =============================================
